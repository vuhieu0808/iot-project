...
- Relay là pulse 1000 ms; cooldown RFID 5000 ms; timeout backend 8000 ms; member session 30000 ms.
...
- ESP32 hiện không có phần cứng RC522/nút/relay trong `diagram.json`; `RELEASE_BUTTON_PIN=-1` và relay count bằng 0 là cấu hình an toàn/chưa đấu thật.
...
10. **Nếu card không tồn tại?**  Service trả denied; không cấp locker và ESP32 không kích relay.
...
13. **Relay được giữ bao lâu?**  `RELAY_PULSE_MS = 1000` ms.
14. **Tại sao code hiện tại chưa mở relay thật?**  `LOCKER_RELAY_COUNT = 0` và relay pin chỉ là placeholder, nên cần map GPIO/phần cứng trước khi triển khai.
15. **Nút release hiện hoạt động chưa?**  Chưa trên cấu hình hiện tại vì `RELEASE_BUTTON_PIN = -1`.
...
27. **Admin thay đổi locker có mở khóa vật lý không?**  Không; route admin chỉ đổi trạng thái database, code hiện tại không publish lệnh relay.
...
29. **Điểm mở rộng khi triển khai 20 locker là gì?**  Khai báo 20 GPIO/driver phù hợp, tăng `LOCKER_RELAY_COUNT`, map số locker–relay và bảo đảm nguồn/cách ly; ESP32 thường cần expander/driver vì GPIO không đủ.
...
- Wokwi wiring hiện chỉ mô phỏng DHT22 và LED/quạt, chưa mô phỏng RC522, nút và relay locker.
